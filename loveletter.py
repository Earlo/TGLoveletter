...
class LoveLetter:
    ...
    def play(self, card, target_player = 0, card_guess = 0):
        ...
        if card == 8: #Princess
            ...
            public_message += "Player " + names[self.turnNumber] + " discarded the Princess and dieded! RIP\n"
    
        if card == 7: #Countess
            public_message += "Player " + names[self.turnNumber] + " discarded the Countess, but y tho?\n"
        ...
        if card == 6: #King
            ...
            public_message += "Player " + names[self.turnNumber] + " discarded the king and switched his hand with " + names[target_player] + "\n"
        ...
        if card == 5: #Prince
            ...
            public_message += "Player " + names[self.turnNumber] + " discarded his prince.\n"

        if card == 4: #Handmaiden
            player[self.turnNumber].handMaiden = True
            public_message += "Player " + names[self.turnNumber] + " discarded his whore Handmaiden.\n"

        if card == 3: #Baron
            if(player[self.turnNumber].cards[0] > player[another_player].cards[0]):
                player[self.turnNumber].alive = False
                public_message += "Player " + names[self.turnNumber] + " discarded his Baron and had a larger dick than " + names[target_player]
            elif(player[self.turnNumber].cards[0] < player[another_player].cards[0]):
                player[another_player] = False
                public_message += "Player " + names[self.turnNumber] + " discarded his Baron and had a smaller dick than " + names[target_player]

        if card == 2: #Priest
            player[self.turnNumber].remove(card)
            public_message += "Player " + names[self.turnNumber] + " discarded his Priest targeting player " + names[target_player]
        ...
        if card == 1: #Guard
            if(guess_card in player[another_player].cards):
                ...
                public_message += "Player " + names[self.turnNumber] + " discarded his guard targeting player " + names[target_player] + " who died for having a " + card_guess
            else:
                public_message += "Player " + names[self.turnNumber] + " discarded his guard targeting player " + names[target_player] + " who did not have a " + card_guess
